...
try {
  ...
  assert.ok(runtimeStart >= 0 && runtimeRouterGuard > runtimeStart, 'model manager must treat Router as virtual before collection/runtime loading');
  assert.match(manager.slice(runtimeRouterGuard, manager.indexOf('const components =', runtimeRouterGuard)), /preflightRouter/);
